...
@router.get("/{user_id}")
async def get_culture(user_id: str, project_id: Optional[int] = None, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")

    if project_id:
        cached = world_cache.get(user_id, project_id, "culture")
        if cached:
            return cached
    ...
    if lore and project_id:
        world_cache.set(user_id, project_id, "culture", lore.dict())

    return lore

@router.post("/{user_id}")
async def update_culture(user_id: str, payload: dict, project_id: Optional[int] = None, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    if effective_project_id:
        world_cache.set(user_id, effective_project_id, "culture", db_lore.dict())

    return db_lore

@router.post("/generate/{user_id}")
async def generate_culture(user_id: str, project_id: int, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    try:
        content = await culture_service.generate(
            project_prompt=project.prompt or project.description or "",
            module_prompt=db_lore.prompt_culture if db_lore else "",
            context=context,
        ...
        db_lore.culture_blob = content
        db_lore.updated_at = datetime.utcnow()
        session.add(db_lore)
        await session.commit()
        await session.refresh(db_lore)

        world_cache.set(user_id, project_id, "culture", db_lore.dict())
