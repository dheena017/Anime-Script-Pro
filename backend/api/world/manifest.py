...
@router.get("/{user_id}")
async def get_manifest(user_id: str, project_id: Optional[int] = None, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
...
@router.post("/{user_id}")
async def update_manifest(user_id: str, payload: dict, project_id: Optional[int] = None, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
...
async def generate_manifest(user_id: str, project_id: int, session: async_session = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
