...
async def get_factions(user_id: str, project_id: Optional[int] = None, session: AsyncSession = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    ...
    return {"content": lore.factions_blob, "prompt": lore.prompt_factions}
...
async def generate_factions(user_id: str, project_id: int, tuning: Optional[dict] = None, session: AsyncSession = Depends(get_async_session), auth_user_id: str = Depends(get_auth_user_id)):
    if user_id != auth_user_id:
        raise HTTPException(status_code=403, detail="Unauthorized")
    ...
    result = await session.execute(statement)
    lore = result.scalars().first()
    module_prompt = lore.prompt_factions if lore else ""

    content = await ai_engine.generate_factions(
