...
interface ManhwaStudioSideBarProps {
  basePath: string;
  handleGenerate?: () => void;
  isLoading?: boolean;
...
export function ManhwaStudioSideBar({
  basePath,
  handleGenerate,
  isLoading,
  rightSidebarOpen,
            ...
            >
              <NavLink
                ...
                className={({ isActive: _isActive }) => cn(
                  ...
                  isActive 
                    ? "text-violet-400 bg-violet-500/10 shadow-[0_0_25px_rgba(139,92,246,0.1)] border border-violet-500/20" 
  ...
  return (
    <motion.aside
      ...
      className={cn(
        ...
        collapsed ? "cursor-pointer hover:bg-violet-500/10" : "cursor-default"
      )}
    ...
    >
      <AnimatePresence mode="wait">
        {!collapsed && (
          ...
          >
            <div className="w-10 h-10 bg-black border border-violet-500/30 rounded-xl flex items-center justify-center shadow-[0_0_25px_rgba(139,92,246,0.2)]">
      ...
      {!collapsed && (
        <div className="p-6 bg-black/60 backdrop-blur-md space-y-4">
          <div className="bg-gradient-to-br from-zinc-900 to-black rounded-3xl p-5 border border-zinc-800 shadow-[0_10px_30px_rgba(0,0,0,0.5)] group relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-violet-500/5 blur-[40px] -z-10 group-hover:bg-violet-500/10 transition-colors" />
            ...
            <Button 
              ...
              disabled={isLoading}
              className={cn(
                "w-full h-11 text-[9px] tracking-[0.3em] bg-violet-600 hover:bg-violet-500 text-white font-black rounded-xl transition-all shadow-lg shadow-violet-600/20",
