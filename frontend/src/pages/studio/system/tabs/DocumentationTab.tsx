    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 bg-zinc-950/50 border border-white/5 p-8 rounded-[2.5rem]">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest leading-none">Access system blueprints and integration protocols</p>
        </div>
        <div className="relative min-w-[300px]">
          ...
          <input 
            ...
            className="w-full bg-zinc-900 border border-white/5 rounded-xl pl-12 pr-6 py-3 text-xs font-black uppercase tracking-widest text-white focus:outline-none focus:border-[#bd4a4a]/50 transition-all"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {docSections.map((section, idx) => (
          ...
          >
            ...
            <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest leading-loose mb-8">{section.desc}</p>
            ...
            <div className="space-y-3">
              {section.items.map(item => (
                <div key={item} className="flex items-center justify-between p-4 bg-zinc-900/40 rounded-2xl border border-white/5 hover:border-[#bd4a4a]/10 transition-all group/item">
                  <span className="text-xs font-black text-zinc-500 uppercase tracking-widest group-hover/item:text-zinc-300">{item}</span>
