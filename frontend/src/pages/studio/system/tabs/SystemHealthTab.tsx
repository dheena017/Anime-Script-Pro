    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-zinc-950/50 border border-white/5 p-8 rounded-[2.5rem]">
        <div className="space-y-2">
          ...
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest leading-none">Real-time telemetry and health monitoring across global nodes</p>
        </div>
        <div className="flex items-center gap-6">
          <div className="text-right">
            <div className="text-xs text-zinc-600 font-black uppercase tracking-widest mb-1">Last Sync</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {metrics.map((m, i) => (
          ...
          >
            ...
            <p className="text-xs font-black text-zinc-600 uppercase tracking-widest mb-1">{m.label}</p>
            <p className="text-lg font-black text-white italic mb-2">{m.value}</p>
            <span className={cn("text-xs font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-full border", m.color.replace('text-', 'border-').replace('500', '500/20'), m.color.replace('text-', 'bg-').replace('500', '500/10'))}>
              {m.status}
            </span>
          </motion.div>
        ))}
          ...
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mt-1">Direct stream from AI Synthesis Core</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-emerald-500 rounded-full animate-ping" />
              <span className="text-xs text-emerald-500 font-black uppercase tracking-widest">Live Flow</span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-zinc-600 uppercase tracking-widest mt-1">Distributed Global Edge Network</p>
              </div>
            </div>
            <span className="text-xs text-blue-500 font-black uppercase tracking-widest">12 Active Nodes</span>
          </div>
              ...
              <div key={i} className="flex items-center justify-between p-5 bg-zinc-900/30 border border-white/5 rounded-2xl hover:border-blue-500/20 transition-all group">
                <div className="flex items-center gap-4">
                  ...
                  <div className="space-y-0.5">
                    <span className="text-xs font-black text-white uppercase tracking-widest leading-none">{node.location}</span>
                    <p className="text-xs font-bold text-zinc-600 uppercase tracking-tighter">{node.id} // SECURE</p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs font-black text-blue-500 uppercase tracking-widest leading-none mb-1">{node.uptime}</div>
                  <div className="text-xs font-bold text-zinc-600 uppercase tracking-widest">{node.status}</div>
                </div>
              </div>
            ))}
          </div>
