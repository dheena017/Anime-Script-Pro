...
export function WorldLoadingPage({ tab, title, description, progress }: WorldLoadingPageProps) {
  const gen = useGeneratorState();
  const isActive = gen.isGeneratingWorld || gen.isGeneratingLore || gen.isGeneratingPowers || gen.isGeneratingFactions || gen.isGeneratingArchitecture || gen.isGeneratingAtlas || gen.isGeneratingCulture || gen.isGeneratingSystems || gen.isLoading;
  ...
  useEffect(() => {
    try { console.debug('[WorldLoadingPage] isActive=', isActive, 'progress=', localProgress); } catch (e) {}
