...
import { Cpu, History, Zap, Users, Building2, Map, Globe, LucideIcon } from 'lucide-react';
...
const TAB_CONFIG: Record<WorldTab, { label: string; subtext: string; icon: LucideIcon; accentColor: string }> = {
  manifest: {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[500px] space-y-10 animate-in fade-in duration-700">
      <div className="relative">
        ...
        {/* Spinning Outer Ring */}
        <div className={`w-24 h-24 border-2 border-white/5 border-t-studio rounded-full animate-spin shadow-[0_0_40px_rgba(6,182,212,0.2)]`} />
        
        {/* Central Icon */}
        <div className="absolute inset-0 m-auto w-10 h-10 flex items-center justify-center">
          <Icon className={`w-10 h-10 ${config.accentColor} animate-pulse drop-shadow-[0_0_15px_rgba(6,182,212,0.5)]`} />
        </div>
      </div>
