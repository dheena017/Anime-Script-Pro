...
import { Music, Heart, Utensils, Languages, BookOpen, Target, Zap, Palette, Users } from 'lucide-react';
import { cn } from '@/lib/utils';

import { useWorldCommandCenter, useCulture } from '../context/WorldCommandCenter';
...
export const CultureTab: React.FC = () => {
  ...
  const handleRefine = async () => {
    try {
      showNotification?.('Societal Refinement active. Harmonizing cultural norms...', 'info');
      await new Promise(resolve => setTimeout(resolve, 1500));
      onContentChange(content + '\n\n*Societal refined: cultural traditions cross-referenced.*');
      showNotification?.('Culture refined successfully.', 'success');
    } catch (err) {
      showNotification?.('Societal Refinement failed. Cultural friction detected.', 'error');
    }
  };
          ...
          <div className="space-y-4 mb-6 animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            <div className="bg-black/40 backdrop-blur-xl border border-white/5 rounded-2xl p-4 flex flex-wrap items-center gap-6 shadow-2xl">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-studio/10 border border-studio/20 rounded-lg">
                <Users className="w-3.5 h-3.5 text-studio" />
                <span className="text-[10px] font-black uppercase tracking-widest text-studio">Societal Tuning</span>
              </div>

              <div className="flex items-center gap-4">
                <TuningOption icon={Languages} label="Tradition" options={['Conservative', 'Progressive', 'Alien']} active="Progressive" />
                <TuningOption icon={BookOpen} label="Values" options={['Honor', 'Survival', 'Knowledge']} active="Knowledge" />
              </div>
            </div>
          </div>
        )}
