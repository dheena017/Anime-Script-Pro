...
export const PowersTab: React.FC<PowersTabProps> = ({
  ...
  const powerStats = React.useMemo(() => {
    ...
    return [
      {
        label: "Source",
        val: findVal(/(?:Source|Origin):\s*(.*)/i, "Primal Aether"),
        icon: Activity,
        color: "text-amber-400",
      },
      {
        label: "Offense",
        val: findVal(/(?:Offense|Attack):\s*(.*)/i, "S-Rank Destruction"),
        icon: Sword,
        color: "text-red-400",
      },
      {
        label: "Defense",
        val: findVal(/(?:Defense|Protection):\s*(.*)/i, "Phase-Shift Barriers"),
        icon: Shield,
        color: "text-blue-400",
      },
      {
        label: "Rarity",
        val: findVal(/(?:Rarity|Tier):\s*(.*)/i, "Legendary Inheritance"),
        icon: Flame,
        ...
        <div className={s.content.contentArea}>
          <div className={s.content.mainColumn}>
            <div
              ...
              style={
                { "--prose-accent-color": "#fbbf24" } as React.CSSProperties
              }
            >
            ...
            <div className={s.content.sidebarCard}>
              ...
              <div className={s.content.sidebarContent}>
                <div className="flex items-center justify-between">
                  ...
                  {isEditing && (
                    <span className="text-xs font-bold text-amber-500/50 uppercase">
                      Modular Prompt
                    </span>
                  )}
                </div>
                ...
                <p className={s.content.sidebarNote}>
                  Focus the AI on specific rules, energy sources, or unique
