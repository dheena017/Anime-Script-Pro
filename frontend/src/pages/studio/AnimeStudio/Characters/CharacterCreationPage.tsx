...
export function CharacterCreationPage() {
  const { prompt, selectedModel, contentType, generatedWorld } =
    useGeneratorState();
  const {
    showNotification,
    setCharacterList,
  ...
  const { setHandlers } = React.useContext(CharacterPageContext);
  ...
  const handleGenerateCharacter = async (focusArchetype?: string) => {
    if (!prompt.trim() && !focusArchetype) {
      showNotification?.(
        "Please enter a story prompt first or select an archetype.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      if (typeof result === "object" && result !== null) {
        ...
        if ("characters" in result) setCharacterList(result.characters);
      }
      showNotification?.(
        focusArchetype
          ? `${focusArchetype} prototype initialized!`
      ...
      console.error(error);
      showNotification?.(
        "Failed to create characters: " + (error.message || "Unknown error"),
        "error",
      );
    } finally {
  ...
  const archetypes = [
    {
      icon: Star,
      title: "Shonen Lead",
      desc: "High determination, latent power, strong moral compass.",
    ...
    },
    {
      icon: Zap,
      title: "Anti-Hero",
      desc: "Dark past, pragmatic methods, complex moral gray areas.",
      color: "text-fuchsia-500",
      border: "border-fuchsia-500/20",
    },
    {
      icon: Fingerprint,
      title: "Mystic Prodigy",
      desc: "Genius intellect, detached personality, hidden lineage.",
      color: "text-cyan-500",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {archetypes.map((archetype, idx) => (
          <Card
            key={idx}
            className={cn(
              "bg-[#030303] border-studio/20 p-8 rounded-[2rem] hover:border-studio/50 transition-all duration-500 group relative overflow-hidden",
              archetype.border,
            )}
          >
            ...
            <archetype.icon
              className={cn(
                "w-12 h-12 mb-6 group-hover:scale-110 transition-all duration-500",
                archetype.color,
              )}
            />
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          <div className="text-center lg:text-left space-y-6">
            ...
            <h2 className="text-3xl font-black text-white uppercase tracking-widest">
              Advanced DNA Sculpting
            </h2>
            <p className="text-zinc-500 text-sm max-w-lg mx-auto lg:mx-0 font-medium italic">
              "Fine-tune the neural weights of your cast. These parameters will
              influence the psychological depth and conflict resolution of the
            ...
            {dnaSliders.map((slider) => (
              <div key={slider.key} className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-mono text-studio">
                    {(dna as any)[slider.key]}%
                  </span>
                </div>
                <div
                  className="relative h-1.5 w-full bg-white/5 rounded-full cursor-pointer group/slider"
                  onClick={(e) => {
                    ...
                    setDna((prev) => ({
                      ...prev,
                      [slider.key]: Math.max(0, Math.min(100, val)),
                    }));
                  }}
                >
                  <div
