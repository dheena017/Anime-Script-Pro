...
export function DynamicsPage() {
  ...
  const { characterRelationships, characterList, characterDynamics } =
    useGeneratorState();
  const { handleGenerateDynamics, isAnalyzingCharacters } = React.useContext(
        ...
        features={[
          {
            ...
            description: "Create cast nodes for relational analysis",
          },
          {
            icon: ArrowRightLeft,
            title: "Thread Mapping",
            description: "Enable ally and rival trajectories",
          },
          {
            icon: Zap,
            title: "Tension Signals",
            description: "Activate conflict simulation layers",
          },
        ]}
        accentColor="rose"
      />
  ...
  const dynamics = characterDynamics || {
    growthArcs: [
      {
        label: "Protagonist Path",
        progress: hasConnections ? 75 : 0,
        color: "bg-fuchsia-500",
      },
      {
        label: "Antagonist Counter",
        progress: hasConnections ? 45 : 0,
        color: "bg-rose-500",
      },
      {
        label: "Sub-plot Variance",
        progress: hasConnections ? 90 : 0,
        color: "bg-studio",
      },
    ],
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] mt-1">
            Growth simulation and conflict mapping
          </p>
        </div>
      </div>
            ...
            {dynamics.growthArcs.map((arc: any, i: number) => (
              <div key={i} className="space-y-4">
                ...
                <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                  <div
                    className={`h-full ${arc.color} transition-all duration-1000`}
          ...
          {hasConnections ? (
            <div className="flex -space-x-4">
              {connections.slice(0, 5).map((_: any, i: number) => (
                <div
                  key={i}
                  className="w-12 h-12 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-zinc-600"
                >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-xl font-black text-white uppercase tracking-widest italic">
                Narrative Tension Dynamics
              </h3>
              <p className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em]">
                {hasConnections
                  ? "Visualizing specific pressure points and emotional collisions"
                  : "Awaiting relationship synthesis"}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {(dynamics.tensionDynamics && dynamics.tensionDynamics.length > 0
              ? dynamics.tensionDynamics
              : [null, null, null, null]
            ).map((dynamic: any, i: number) => (
                ...
                <div className="relative p-8 bg-black/40 border border-white/5 rounded-[2rem] space-y-6 text-center backdrop-blur-sm">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">
                      {dynamic
                        ? `${dynamic.source} vs ${dynamic.target}`
                        : "Awaiting Thread"}
                    </p>
                  </div>
