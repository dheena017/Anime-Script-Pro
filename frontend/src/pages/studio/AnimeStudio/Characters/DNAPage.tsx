...
export function DNAPage() {
  ...
  const { characterList, characterDNA } = useGeneratorState();
  const { handleGenerateDNA, isAnalyzingCharacters } = React.useContext(
    CharacterTabActionsContext,
  );
  const hasCast = characterList && characterList.length > 0;
        ...
        features={[
          {
            ...
            description: "Generate core identities and personality traits",
          },
          {
            icon: ShieldCheck,
            title: "Logic Consistency",
            description: "Enable stability and consistency metrics",
          },
          {
            icon: Search,
            title: "Deep Analysis",
            description: "Unlock detailed profiling across the full cast",
          },
        ]}
        accentColor="cyan"
      />
  ...
  const dna = characterDNA || {
    cognitiveLoad: Math.min(100, characterList.length * 15),
    emotionalFlux: Math.min(100, 45 + characterList.length * 5),
    ...
    <div className="space-y-12 pb-20">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] mt-1">
            Deep psychological and personality profiling
          </p>
        </div>
      </div>
        ...
        {[
          {
            icon: Brain,
            label: "Mental Depth",
            value: `${dna.cognitiveLoad}%`,
            color: "text-studio",
          },
          {
            icon: Activity,
            label: "Emotional Intensity",
            value: `${dna.emotionalFlux}%`,
            color: "text-fuchsia-400",
          },
          {
            icon: ShieldCheck,
            label: "Narrative Importance",
            value: dna.narrativeRole,
            color: "text-emerald-400",
          },
          {
            icon: BarChart3,
            label: "Conflict Impact",
            value: dna.conflictBalance,
            color: "text-amber-400",
          },
        ].map((stat, idx) => (
          <Card
            key={idx}
            className="bg-black/40 border border-white/5 p-6 rounded-2xl flex flex-col items-center gap-4 hover:border-studio/30 transition-all backdrop-blur-md"
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <Card className="bg-[#030303] border-studio/20 p-10 rounded-[3rem] relative overflow-hidden group">
          ...
          <h3 className="text-xl font-black text-white uppercase tracking-widest mb-8">
            Narrative Synapse Scan
          </h3>
          ...
          <div className="mt-12 p-6 bg-studio/5 border border-studio/10 rounded-2xl">
            <p className="text-xs font-black text-studio uppercase tracking-widest text-center">
              {characterDNA ? "Analysis Path Active" : "Input Required"}
            </p>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:col-span-2">
          <div className="p-10 bg-black/40 border border-white/5 rounded-[3rem] space-y-8 backdrop-blur-md">
            ...
            <div className="h-48 flex items-end gap-3 px-4">
              {dna.weightDistribution.map((h: number, i: number) => (
                <div
                  key={i}
                  className="flex-1 bg-gradient-to-t from-blue-500/40 to-blue-500/10 rounded-t-xl transition-all"
                  style={{ height: `${h}%` }}
                />
              ))}
            </div>
          ...
          <div className="space-y-4">
            {[
              ...
              <div
                key={i}
                className="p-8 bg-black/40 border border-white/5 rounded-[2rem] flex items-center justify-between group hover:bg-white/[0.03] transition-all backdrop-blur-sm"
              >
                ...
                <span className="text-xs font-black text-blue-400 uppercase tracking-widest italic">
                  {item.score}
                </span>
              </div>
            ))}
