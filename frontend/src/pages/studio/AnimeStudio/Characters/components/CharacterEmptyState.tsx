...
const features = [
  {
    icon: Users,
    title: "Registry",
    description: "Core character dossiers and archetypes",
    color: "cyan",
  },
  {
    icon: Mic2,
    title: "Voice",
    description: "Vocal rhythm and dialogue archetypes",
    color: "blue",
  },
  {
    icon: Swords,
    title: "Combat",
    description: "Power systems and signature techniques",
    color: "red",
  },
  {
    icon: TrendingUp,
    title: "Arcs",
    description: "Character growth and moral roadmaps",
    color: "fuchsia",
  },
  {
    icon: GitBranch,
    title: "Dynamics",
    description: "Social standing and group etiquette",
    color: "orange",
  },
  {
    icon: Workflow,
    title: "Relationships",
    description: "Holistic social web and matrix",
    color: "indigo",
  },
];

export const CharacterEmptyState: React.FC<CharacterEmptyStateProps> = ({
  onLaunch,
  onLoadDemo,
  isGenerating,
}) => {
        ...
        <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
          ...
          <div className="flex-1 space-y-4 w-full">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">
                  Adjust number of souls to sequence
                </p>
              </div>
              <div className="px-4 py-1 bg-studio/10 border border-studio/20 rounded-xl">
                <span className="text-xl font-black text-studio tracking-tighter">
                  {numCharacters}
                </span>
                <span className="text-xs font-black text-studio/40 ml-1 uppercase">
                  Units
                </span>
              </div>
            </div>
            ...
            <Slider
              value={[numCharacters]}
              onValueChange={(val) =>
                setNumCharacters(Array.isArray(val) ? val[0] : val)
              }
              min={3}
              max={30}
              step={1}
