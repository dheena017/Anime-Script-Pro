...
import { Users, Heart, Swords, Shield, Zap, Lock, Minus } from "lucide-react";
...
export const RelationshipMatrix = React.memo(() => {
  ...
  // Extract character names from characterList or fallback to characterProfiles
  const characterNames = React.useMemo(() => {
    if (characterList && characterList.length > 0) {
      return characterList.map((c) => c.name).slice(0, 8);
    }

    if (!characterProfiles) return [];
    const names = new Set<string>();
    const lines = characterProfiles.split("\n");
    lines.forEach((line: string) => {
      const match = line.match(/^\d*\.?\s*\*\*?([^*]+)\*\*?/);
      if (match) names.add(match[1].trim());
      else if (line.includes(":")) {
        const potentialName = line.split(":")[0].trim();
        if (potentialName.length < 20 && potentialName.length > 2)
          names.add(potentialName);
      }
    });
    return Array.from(names).slice(0, 8);
  }, [characterList, characterProfiles]);

  // Parse relationships from structured data or fallback to string searching
  const relationsMap = React.useMemo(() => {
    const map: Record<string, any> = {};

    // Prioritize structured data from characterData
    if (
      characterData?.relationships &&
      Array.isArray(characterData.relationships)
    ) {
      characterData.relationships.forEach((rel: any) => {
      ...
      lines.forEach((line) => {
        characterNames.forEach((nameA) => {
          characterNames.forEach((nameB) => {
            ...
            if (
              line.toLowerCase().includes(nameA.toLowerCase()) &&
              line.toLowerCase().includes(nameB.toLowerCase())
            ) {
              let type = "Neutral";
              ...
              else if (line.includes("Apprentice") || line.includes("Master"))
    ...
    return map;
  }, [characterRelationships, characterNames, characterData]);
  ...
  const getRelationIcon = (type: string) => {
    switch (type) {
      ...
      case "Love":
        return <Heart className="w-5 h-5 text-fuchsia-400" />;
      case "Secret":
        return <Lock className="w-5 h-5 text-zinc-500" />;
      case "Familial":
        return <Users className="w-5 h-5 text-blue-400" />;
      case "Betrayal":
        return <Minus className="w-5 h-5 text-amber-600 rotate-45" />;
      case "Stalker":
        return <Zap className="w-5 h-5 text-purple-600 animate-pulse" />;
      default:
        return <Minus className="w-4 h-4 text-zinc-800" />;
    }
  };
        ...
        <div className="min-w-[600px]">
          <div className="grid grid-cols-[140px_repeat(8,1fr)] gap-2">
            ...
            {characterNames.map((name, i) => (
              <div
                key={i}
                className="h-20 flex flex-col items-center justify-end pb-4 bg-zinc-950/20 rounded-t-2xl border-x border-t border-zinc-900/50"
              >
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        rowIndex === characterNames.length - 1 &&
                          "rounded-b-none",
                        colIndex === characterNames.length - 1 &&
                          "rounded-r-none",
                      )}
                    >
                      {isSelf ? (
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="bg-zinc-950 border-studio text-studio p-3 max-w-xs shadow-2xl">
                            <p className="text-xs font-black uppercase tracking-widest mb-1 underline">
                              {relation.type}
                            </p>
