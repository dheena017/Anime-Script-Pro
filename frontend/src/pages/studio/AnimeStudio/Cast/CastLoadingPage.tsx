...
import { Loader2, Users, Fingerprint, Brain, ShieldCheck, UserPlus } from 'lucide-react';
...
const TAB_META: Record<string, { title: string; description: string; icon: any; color: string; accentColor: string; borderColor: string; bgColor: string; shadowColor: string }> = {
  registry: { title: 'Creating Characters', description: 'Building character profiles and backstories', icon: Users, color: 'text-purple-400', accentColor: 'text-purple-400', borderColor: 'border-purple-500/20', bgColor: 'bg-purple-500/5', shadowColor: 'rgba(168, 85, 247, 0.1)' },
  dna: { title: 'Analyzing Character Traits', description: 'Mapping personality and behavioral patterns', icon: Fingerprint, color: 'text-pink-400', accentColor: 'text-pink-400', borderColor: 'border-pink-500/20', bgColor: 'bg-pink-500/5', shadowColor: 'rgba(236, 72, 153, 0.1)' },
  dynamics: { title: 'Mapping Relationships', description: 'Determining character relationships and group dynamics', icon: Brain, color: 'text-violet-400', accentColor: 'text-violet-400', borderColor: 'border-violet-500/20', bgColor: 'bg-violet-500/5', shadowColor: 'rgba(167, 139, 250, 0.1)' },
  integrity: { title: 'Checking Consistency', description: 'Verifying character and narrative consistency', icon: ShieldCheck, color: 'text-blue-400', accentColor: 'text-blue-400', borderColor: 'border-blue-500/20', bgColor: 'bg-blue-500/5', shadowColor: 'rgba(59, 130, 246, 0.1)' },
  'add-lead': { title: 'Adding Character', description: 'Adding your custom character to the cast', icon: UserPlus, color: 'text-emerald-400', accentColor: 'text-emerald-400', borderColor: 'border-emerald-500/20', bgColor: 'bg-emerald-500/5', shadowColor: 'rgba(16, 185, 129, 0.1)' },
};
...
export function CastLoadingPage({ tab, title, description, progress }: CastLoadingPageProps) {
    ...
    <div className="w-full py-20 flex items-center justify-center min-h-[500px]">
      <div className={`w-full max-w-3xl rounded-[2rem] border ${meta.borderColor} bg-[#050505] px-8 py-16 text-center backdrop-blur-sm`} style={{ boxShadow: `0 0 60px ${meta.shadowColor}` }}>
        ...
        <div className="mx-auto mb-6 h-1.5 w-full max-w-md overflow-hidden rounded-full bg-white/5 border border-white/10">
          <div className={`h-full rounded-full transition-all duration-300 ${meta.bgColor}`} style={{ width: `${typeof progress === 'number' ? progress : localProgress}%` }} />
        </div>
