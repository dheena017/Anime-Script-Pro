...
export const CastCard = React.memo<CastCardProps>(({
  ...
  const getCombatTier = (): string => toText(character.powerSystem?.powerTier || 'Tier A');
  const getCombatAbility = (): string => toText(character.powerSystem?.signatureAbility || character.combatStyle || 'Aether Reactor Kinetic Strike');
  const getCombatDetails = (): string => toText(character.powerSystem?.limitations || (character.combatStyle ? `Combat style: ${character.combatStyle}` : 'Operational limits within default parameters.'));

  const getVoiceRhythm = (): string => toText(character.speakingStyle?.dialogueRhythm || (typeof character.speakingStyle === 'string' ? 'Rhythmic' : 'Melodic'));
  const getVoiceArchetype = (): string => toText(character.speakingStyle?.voiceArchetype || (typeof character.speakingStyle === 'string' ? character.speakingStyle : 'Standard Vocoder Model'));
  const getVoiceCatchphrase = (): string => {
    if (Array.isArray(character.speakingStyle?.catchphrases) && character.speakingStyle.catchphrases.length > 0) {
  ...
  const getVfxMovement = (): string => toText(character.technicalModel?.movementStyle || 'High-Velocity Trace');
  const getVfxSignature = (): string => toText(character.technicalModel?.vfxSignature || (typeof character.appearance === 'string' ? character.appearance : character.appearance?.silhouette) || character.visuals || 'Organic particle trails');
  const getVfxLighting = (): string => toText(character.technicalModel?.lightingLogic || 'High-Contrast Backlighting');

  const getPsychologyArc = (): string => toText(character.narrative?.arcType || 'Evolution');
  const getPsychologyWound = (): string => toText(character.psychologyProfile?.coreWound || character.psychology || character.personality || 'Internal conflict uncatalogued.');
  const getPsychologyCoping = (): string => toText(character.psychologyProfile?.copingMechanism || 'Strategic isolation');

  // Resilient production-readiness calculator based on populated data segments
  const readiness = [
    character.speakingStyle?.voiceArchetype || (typeof character.speakingStyle === 'string'),
    character.powerSystem?.signatureAbility || character.combatStyle,
    character.psychologyProfile?.coreWound || character.psychology || character.personality,
    character.technicalModel?.vfxSignature || character.appearance || character.visuals
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const imageUrl = await generateSceneImage(character.visualPrompt, undefined, isDemoMode);
      if (imageUrl) {
        onUpdate({ imageUrl });
        showNotification?.('Profile visualization compiled successfully!', 'success');
      }
    } catch (error: any) {
      console.error("Failed to generate character image:", error);
      showNotification?.(`Visualization failed: ${error?.message || 'Unknown issue'}`, 'error');
    } finally {
      ...
      <div className="px-10 pb-10 -mt-14 relative z-10">
        <div className="flex flex-col md:flex-row gap-8 items-end mb-8">
          ...
          <div className="relative group/avatar shrink-0">
            ...
            <div className="w-36 h-36 rounded-[2.2rem] bg-zinc-950 border border-white/10 flex items-center justify-center overflow-hidden relative z-10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] group-hover/avatar:border-studio/50 transition-all duration-700">
              <AnimatePresence mode="wait">
                {isGeneratingImage ? (
                  <motion.div key="loading" className="flex flex-col items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-studio uppercase tracking-widest animate-pulse">Scanning DNA</span>
                  </motion.div>
                ) : character.imageUrl ? (
                  <motion.img 
              ...
              {!isGeneratingImage && (
                <div className={cn(
                  "absolute inset-0 transition-all duration-300 flex items-center justify-center p-4",
                  character.imageUrl 
                    ? "bg-black/50 backdrop-blur-xs opacity-0 group-hover/avatar:opacity-100" 
                    : "bg-black/85 opacity-0 group-hover/avatar:opacity-100"
                )}>
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={handleGenerateImage}
                    className="w-full h-full text-[10px] font-black text-white hover:text-studio hover:bg-transparent flex flex-col gap-3 cursor-pointer"
                  >
                    <Camera className="w-6 h-6" />
                    {character.imageUrl ? 'REGENERATE VIZ' : 'GENERATE VIZ'}
                  </Button>
                </div>
              )}
            </div>
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
           <div className="px-4 py-3 bg-zinc-950/60 border border-white/5 rounded-2xl flex items-center gap-3">
             ...
             <div className="min-w-0 flex-1">
               <div className="text-[9px] font-black text-zinc-600 uppercase tracking-wider">FACTION</div>
               <div className="text-xs font-black text-zinc-300 truncate uppercase tracking-wide">
                 {toText(character.worldAlignment?.factionAffiliation, 'Independent')}
               </div>
             </div>
           </div>
           ...
           <div className="px-4 py-3 bg-zinc-950/60 border border-white/5 rounded-2xl flex items-center gap-3">
             ...
             <div className="min-w-0 flex-1">
               <div className="text-[9px] font-black text-zinc-600 uppercase tracking-wider">ORIGIN</div>
               <div className="text-xs font-black text-zinc-300 truncate uppercase tracking-wide">
                 {toText(character.worldAlignment?.geographicOrigin, 'Unknown Sector')}
               </div>
             </div>
           </div>
           ...
           <div className="px-4 py-3 bg-zinc-950/60 border border-white/5 rounded-2xl flex items-center gap-3">
             ...
             <div className="min-w-0 flex-1">
               <div className="text-[9px] font-black text-zinc-600 uppercase tracking-wider">BIOLOGY</div>
               <div className="text-xs font-black text-zinc-300 truncate uppercase tracking-wide">
                 {character.age ? `${character.age} yrs` : 'Age N/A'} | {character.gender || 'Classified'}
               </div>
             </div>
           </div>
        </div>
        ...
        <div className="mb-8 p-5 bg-zinc-950/80 rounded-3xl border border-white/5 space-y-3">
           <div className="flex items-center justify-between">
              <span className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Production Ready Factor</span>
           ...
           <div className="p-6 bg-zinc-950/40 rounded-[2rem] border border-white/5 hover:border-studio/25 hover:bg-zinc-900/20 transition-all duration-500 space-y-3 flex flex-col justify-between">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                 <div className="flex items-center gap-2 text-studio">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">Combat Telemetry</span>
                 </div>
           ...
           <div className="p-6 bg-zinc-950/40 rounded-[2rem] border border-white/5 hover:border-fuchsia-500/25 hover:bg-zinc-900/20 transition-all duration-500 space-y-3 flex flex-col justify-between">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                 <div className="flex items-center gap-2 text-fuchsia-500">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">Voice Engine</span>
                 </div>
              ...
              <div className="space-y-2">
                 <div className="text-[11px] font-black text-white uppercase tracking-wider">
                   ARCHETYPE: <span className="text-fuchsia-400 font-black">{getVoiceArchetype()}</span>
                 </div>
                 <div className="text-[10px] text-zinc-500 font-medium leading-relaxed italic">
                   {getVoiceCatchphrase()}
                 </div>
           ...
           <div className="p-6 bg-zinc-950/40 rounded-[2rem] border border-white/5 hover:border-cyan-500/25 hover:bg-zinc-900/20 transition-all duration-500 space-y-3 flex flex-col justify-between">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                 <div className="flex items-center gap-2 text-cyan-400">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">Visual VFX Model</span>
                 </div>
              ...
              <div className="space-y-2">
                 <div className="text-[11px] font-black text-white uppercase tracking-wider">
                   VFX: <span className="text-cyan-400 font-black">{getVfxSignature()}</span>
                 </div>
           ...
           <div className="p-6 bg-zinc-950/40 rounded-[2rem] border border-white/5 hover:border-orange-500/25 hover:bg-zinc-900/20 transition-all duration-500 space-y-3 flex flex-col justify-between">
              <div className="flex items-center justify-between border-b border-white/5 pb-2">
                 <div className="flex items-center gap-2 text-orange-500">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-wider">Psychology Model</span>
                 </div>
              ...
              <div className="space-y-2">
                 <div className="text-[11px] font-black text-white uppercase tracking-wider truncate">
                   WOUND: <span className="text-orange-400 font-black">{getPsychologyWound()}</span>
                 </div>
                 <div className="text-[10px] text-zinc-500 font-medium leading-relaxed">
                   {getPsychologyCoping()}
                 </div>
                ...
                <div className="flex gap-1 flex-1">
                  {[...Array(5)].map((_, i) => (
                    <motion.div
                      ...
                      animate={{
                        scaleY: [1, 1.6, 1],
                        backgroundColor: ["rgba(249,115,22,0.3)", "rgba(249,115,22,1)", "rgba(249,115,22,0.3)"]
                      }}
                      transition={{
                        duration: 1.8,
                        repeat: Infinity,
        ...
        <div className="mb-4 p-6 bg-zinc-950/40 rounded-[2rem] border border-white/5 flex flex-col gap-3">
          <div className="flex items-center gap-2 text-studio">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider">Main Objective</span>
          </div>
