...
export const AddLeadTab: React.FC = () => {
  ...
  if (!showForm) {
    return (
      <StudioEmptyState
        ...
        features={[
          { icon: Fingerprint, title: 'Identity Seed', description: 'Create the foundational lead profile' },
          { icon: Brain, title: 'Motivation Core', description: 'Define archetype and narrative drive' },
          { icon: Sparkles, title: 'Cast Bootstrap', description: 'Use this lead as the first registry anchor' }
        ]}
        accentColor="amber"
      />
    );
  ...
  const handleBrainstorm = async () => {
    if (!name && !archetype) {
      showNotification?.("Please provide at least a name or archetype for brainstorming.", "info");
      return;
    }
    ...
    try {
      ...
      const result = await generateCharacters(userReq, selectedModel, contentType, generatedWorld || undefined, 1);
      
      if (typeof result === 'object' && result.characters && result.characters.length > 0) {
        const char = result.characters[0];
        setName(char.name);
        setArchetype(char.archetype);
        setGoal(char.goal);
        showNotification?.("AI has synthesized a character profile.", "success");
      }
    } catch (e) {
      console.error(e);
  ...
  const handleAddLead = () => {
    ...
    setCastList([...(castList || []), newCharacter]);
    showNotification?.(`${name} has been manifested in the registry.`, "success");
    ...
    <div className="max-w-3xl mx-auto py-12 space-y-12">
      ...
      <div className="space-y-6">
         <div className="grid grid-cols-2 gap-6">
           ...
           <div className="space-y-2">
             ...
             <div className="relative">
               ...
               >
                 <Sparkles className={cn("w-5 h-5", isBrainstorming && "animate-pulse")} />
               </button>
             </div>
           </div>
         </div>
         ...
         <div className="flex gap-4">
           ...
           >
              {isBrainstorming ? <RefreshCcw className="w-5 h-5 animate-spin" /> : <Sparkles className="w-5 h-5" />}
              AI Brainstorm
           </Button>
           <Button 
