...
export const AnimaticTab: React.FC<AnimaticTabProps> = ({
  scenes: propsScenes,
  ...
  const [activeTheaterIndex, setActiveTheaterIndex] = useState<number | null>(
    null,
  );
  ...
  const activeScene =
    activeTheaterIndex !== null ? scenes[activeTheaterIndex] : null;
  const activeVideoUrl = activeScene
    ? videoData[activeScene.originalIndex]
    : null;
                ...
                <div className="lg:col-span-2 relative aspect-video w-full bg-black rounded-2xl overflow-hidden border border-white/10 flex items-center justify-center shadow-2xl">
                  ...
                  {activeScene && (
                    <div className="absolute top-6 left-6 bg-black/60 backdrop-blur-md border border-white/10 text-white font-mono text-[10px] font-black px-3 py-1.5 rounded-lg z-30 flex items-center gap-2">
                      ...
                      Scene {String(activeTheaterIndex! + 1).padStart(2, "0")} —{" "}
                  ...
                  {isPlayingAll && (
                    <div
                      className="absolute bottom-0 left-0 h-1.5 bg-emerald-500 transition-all duration-100 shadow-emerald"
                      style={{ width: `${theaterPlaybackProgress}%` }}
                    />
                  )}
                </div>
                    ...
                    <div className="space-y-4 animate-in fade-in duration-300">
                      ...
                      {activeScene.soulFocus && (
                        <div>
                          ...
                          <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider block mt-1">
                            👤 {activeScene.soulFocus}
                          </span>
                        </div>
                      )}
                      ...
                      {activeScene.emotionalKey && (
                        <div>
                          ...
                          <span className="text-xs font-bold text-orange-400 uppercase tracking-wider block mt-1">
                            🔥 {activeScene.emotionalKey}
                          </span>
                        </div>
                      )}
      ...
      {!hasVideos && scenes.length > 0 && (
        <div className="text-center py-8 border border-dashed border-white/5 bg-white/[0.01] rounded-3xl max-w-4xl mx-auto">
          <p className="text-xs font-black text-zinc-600 uppercase tracking-widest">
            Generate videos from the VIDEO tab to activate full animatic
            playback
          </p>
        </div>
      )}
