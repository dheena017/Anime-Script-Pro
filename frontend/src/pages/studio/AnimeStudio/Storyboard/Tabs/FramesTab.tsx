...
export const FramesTab = React.memo<Partial<FramesTabProps>>((props) => {
  ...
  const handleSetEditForm = React.useCallback(
    (
      form: Partial<Scene> | ((prevState: Partial<Scene>) => Partial<Scene>),
    ) => {
      ...
      <aside className={s.content.sidebar + " space-y-8"}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-4 gap-2">
            {scenes.map((scene, i) => (
              ...
              >
                <span className="text-[10px] font-black text-zinc-600 group-hover:text-orange-400">
                  {i + 1}
                </span>
              </motion.button>
            ))}
        ...
        <div className="p-6 bg-gradient-to-br from-orange-500/10 to-studio/10 border border-white/5 rounded-[2rem] space-y-4">
          ...
          <p className="text-[10px] font-medium text-zinc-500 leading-relaxed uppercase">
            The visual sequence is now locked into the production pipeline. Use
            the navigator to verify frame consistency across keys.
          </p>
        </div>
      </aside>
    </div>
