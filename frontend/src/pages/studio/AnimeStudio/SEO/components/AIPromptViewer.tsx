...
import { Terminal, Copy, Check, ChevronDown, ChevronUp, Cpu } from 'lucide-react';
...
export const AIPromptViewer: React.FC<AIPromptViewerProps> = ({
  ...
  const getActivePromptData = () => {
    const dummyScript = script || "[Your Anime Script Content will be inserted here when you write a script...]";
    ...
    switch (activeTab) {
      ...
      case 'description':
        return {
          ...
          prompt: YOUTUBE_DESCRIPTION_GENERATION_PROMPT(contentType, dummyScript),
          theme: 'fuchsia' as const
        };
      case 'alt':
        return {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 relative z-10">
          <div className="flex items-center gap-3">
            <div className="relative flex items-center justify-center p-2.5 rounded-xl bg-white/[0.02] border border-white/5">
              ...
              <span className={cn("absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full border-2 border-black animate-pulse", themeStyles.pulse)} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h4 className="text-xs font-black uppercase tracking-[0.2em] text-white">System Prompt Console</h4>
                <span className={cn("px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider border", themeStyles.badge)}>
                  Gemini-3.1-Pro
                </span>
              </div>
              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider mt-0.5">
                Exposing system prompt & strategic guidelines for the {activeTab.toUpperCase()} module
              </p>
            </div>
          </div>
            ...
            <AnimatePresence>
              {isOpen && (
                ...
                >
                  ...
                  >
                    {copied ? <Check className="w-3.5 h-3.5 text-green-400 animate-in zoom-in-50" /> : <Copy className="w-3.5 h-3.5" />}
            ...
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-3">
                  ...
                  <span className={cn("text-[10px] font-bold uppercase tracking-wider", themeStyles.text)}>
                    {title}
                  </span>
                </div>
                ...
                <div className="relative">
                  ...
                  <pre 
                    className="p-5 rounded-2xl bg-black/60 border border-white/5 font-mono text-[11px] leading-relaxed text-zinc-300 overflow-y-auto max-h-[380px] scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent whitespace-pre-wrap select-text selection:bg-white/10 selection:text-white"
                  >
                    {prompt.trim()}
                  </pre>
                </div>
                ...
                <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest text-right">
                  System triggers automatic payload parsing on response acquisition.
                </p>
              </div>
            </motion.div>
          )}
