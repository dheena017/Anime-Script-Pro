...
export const AltTextTab: React.FC<AltTextTabProps> = ({
  content,
  isGenerating,
  onGenerate,
}) => {
    ...
    <div className="space-y-6">
      <Card
        className={cn(
          s.cardContainer,
          content
            ? "border-cyan-500/30 shadow-[0_0_40px_rgba(56,189,248,0.1)]"
          ...
          {isGenerating ? (
            <div className={s.loadingStateContainer + " text-cyan-500"}>
              <div
                className={
                  s.loadingSpinner +
                  " border-cyan-500/20 border-t-cyan-500 shadow-[0_0_20px_rgba(56,189,248,0.3)]"
                }
              />
              <h4 className={s.loadingTitle + " text-cyan-400"}>
                Alt Text Generator Online
              </h4>
              <p className={s.loadingText}>
                Creating descriptive, accessibility-ready alt captions for your
                visual content.
              </p>
            </div>
          ) : content ? (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000">
              <ReactMarkdown
                components={{
                  ...
                  strong: ({ node, ...props }) => (
                    <strong className="text-cyan-300 font-black" {...props} />
                  ),
                  em: ({ node, ...props }) => (
            ...
            <div className={s.emptyStateContainer}>
              <div
                className={
                  s.emptyIconBox +
                  " group-hover/empty:border-cyan-500/30 group-hover/empty:bg-cyan-500/5"
                }
              >
                <Globe className="w-8 h-8 opacity-20 group-hover/empty:opacity-60 transition-opacity" />
              </div>
              <p className={s.emptyText}>
                Generate descriptive alt text for storyboard and marketing
                visuals.
              </p>
            </div>
          )}
