...
export const TagsTab: React.FC<TagsTabProps> = ({ content, isGenerating, onGenerate }) => {
  ...
  let tags: string[] = ['Anime', 'Storytelling', 'AI Production', 'Animation', 'Tutorial', 'Nexus', 'Visual DNA'];
  if (content) {
    try {
          ...
          <div className="relative z-10 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <span className="text-xs font-bold uppercase tracking-widest">{copiedAll ? 'Copied' : 'Copy All'}</span>
              </Button>
            </div>
            <div className={s.tagsContainer}>
              {tags.map((tag, i) => (
                <button 
                  ...
                  className={s.tagItem + ' bg-fuchsia-500/10 border-fuchsia-500/20 hover:border-fuchsia-400/50 hover:bg-fuchsia-500/20 text-fuchsia-300 transition-all active:scale-95 duration-200 cursor-pointer'}
                >
                  {copiedTagIndex === i ? <Check className="w-3 h-3 text-green-400" /> : <Tag className="w-3 h-3 text-fuchsia-400" />}
                  {tag}
                </button>
              ))}
            </div>
            <p className="text-xs text-zinc-500 italic mt-8 leading-relaxed">
              These tags are optimized for YouTube, TikTok, and Instagram discovery algorithms based on your script content. Click any tag to copy it individually.
            </p>
          </div>
        ) : (
