            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              {data.title_suggestions && data.title_suggestions.length > 0 && (
                <div className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-cyan-400">Title Options</h4>
                  <div className="grid grid-cols-1 gap-3">
                    {data.title_suggestions.map((title: string, i: number) => (
                      <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/20 transition-all group">
                        <span className="text-sm font-bold text-zinc-200 tracking-wide">{title}</span>
                        <Button 
                          variant="ghost" 
                          size="icon" 
                        ...
                        >
                          {copiedTitleIndex === i ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                        </Button>
                      </div>
                    ))}
                  </div>
              ...
              {data.meta_description && (
                <div className="space-y-4">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] text-cyan-400">Meta Description</h4>
                  <div className="p-6 rounded-2xl bg-white/[0.01] border border-white/5 relative">
                    <div className="absolute top-4 right-4">
                      ...
                      >
                        {copiedDesc ? <Check className="w-3.5 h-3.5 text-green-400" /> : <Copy className="w-3.5 h-3.5" />}
                      </Button>
                    </div>
                    <p className="text-sm text-zinc-400 leading-relaxed font-medium pr-10">{data.meta_description}</p>
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {data.primary_keywords && data.primary_keywords.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="text-xs font-black uppercase tracking-[0.2em] text-cyan-400">Primary Keywords</h4>
                    <div className="flex flex-wrap gap-2">
                      {data.primary_keywords.map((kw: string, i: number) => (
                        <div key={i} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-cyan-500/10 border border-cyan-500/20 text-cyan-300">
                          {kw}
                        </div>
                      ))}
                    </div>
                ...
                {data.secondary_keywords && data.secondary_keywords.length > 0 && (
                  <div className="space-y-4">
                    <h4 className="text-xs font-black uppercase tracking-[0.2em] text-cyan-400">Secondary Keywords</h4>
                    <div className="flex flex-wrap gap-2">
                      {data.secondary_keywords.map((kw: string, i: number) => (
                        <div key={i} className="px-3 py-1.5 rounded-lg text-xs font-bold bg-white/[0.02] border border-white/5 text-zinc-400">
                          {kw}
                        </div>
                      ))}
                    </div>
