    ...
    <div className="space-y-6">
      <Card
        className={cn(
          s.cardContainer,
          content
            ? "border-orange-500/30 shadow-[0_0_40px_rgba(249,115,22,0.1)]"
          ...
          {isLoading ? (
            ...
            <div className={s.loadingStateContainer + " text-orange-500"}>
              <div
                className={
                  s.loadingSpinner +
                  " border-orange-500/20 border-t-orange-500 shadow-[0_0_20px_rgba(249,115,22,0.3)]"
                }
              />
              <h4 className={s.loadingTitle + " text-orange-400"}>
                Amplification Blueprints Compiling
              </h4>
              <p className={s.loadingText}>
                Generating viral hooks, audience retention checkpoints, and
                pacing guidelines.
              </p>
            </div>
          ) : content ? (
            <div className="relative group animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
              <div className="flex items-center justify-between border-b border-white/5 pb-4">
                <span className="text-xs font-black uppercase tracking-[0.2em] text-orange-400">
                  Episodic Amplification Blueprint
                </span>
              ...
              <div className="prose prose-invert max-w-none">
                <ReactMarkdown
                  components={{
                    ...
                    h2: ({ node, ...props }) => (
                      <h2
                        className="text-lg font-black text-orange-500 uppercase tracking-widest mb-4 mt-8"
                        {...props}
                      />
                    ),
                    h3: ({ node, ...props }) => (
                    ...
                    strong: ({ node, ...props }) => (
                      <strong
                        className="text-orange-300 font-black"
                        {...props}
                      />
                    ),
                    em: ({ node, ...props }) => (
                    ...
                    blockquote: ({ node, ...props }) => (
                      <blockquote
                        className="border-l-2 border-orange-500 pl-4 italic text-zinc-400 bg-orange-500/5 p-3 rounded-r-lg my-4"
                        {...props}
                      />
                    ),
                    code: ({
                      ...
                      return inline ? (
                        <code
                          className="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-xs font-mono text-orange-300"
                          {...props}
                        >
                          {children}
                        </code>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center max-w-md mx-auto space-y-2 mb-8">
                <h3 className="text-xs font-black text-orange-400 uppercase tracking-[0.2em]">
                  Select Amplification Focus
                </h3>
                <p className="text-xs text-zinc-500 uppercase tracking-widest leading-relaxed">
                  Choose a growth vector to synthesize a high-engagement
                  promotional strategy for this script.
                </p>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 leading-relaxed uppercase tracking-wider line-clamp-3">
                          {strat.description}
                        </p>
                      </div>
