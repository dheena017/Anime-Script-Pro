...
interface GeneratorAutoSaveParams {
  userId?: string;
  production: any;
...
export function useGeneratorAutoSave(params: GeneratorAutoSaveParams) {
  const {
    userId,
    production,
    resolveProjectId,
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        ...
        if (generatedWorld) {
          await worldApi.manifest.update(userId, generatedWorld, promptLore, projectId);
        }
        if (generatedWorldLore) {
          await worldApi.history.update(userId, generatedWorldLore, promptLore, projectId);
        }
        if (generatedWorldPowers) {
          await worldApi.powers.update(userId, generatedWorldPowers, promptPowers, projectId);
        }
        if (generatedWorldFactions) {
          await worldApi.factions.update(userId, generatedWorldFactions, promptFactions, projectId);
        }
        if (generatedWorldArchitecture) {
          await worldApi.architecture.update(userId, generatedWorldArchitecture, promptArchitecture, projectId);
        }
        if (generatedWorldAtlas) {
          await worldApi.atlas.update(userId, generatedWorldAtlas, promptAtlas, projectId);
        }
        if (generatedWorldCulture) {
          await worldApi.culture.update(userId, generatedWorldCulture, promptCulture, projectId);
        }
        if (generatedWorldSystems) {
          await worldApi.systems.update(userId, generatedWorldSystems, promptSystems, projectId);
        }
