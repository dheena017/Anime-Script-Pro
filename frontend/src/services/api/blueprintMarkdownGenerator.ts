...
function renderFrame(promptObject: any, frameIndex: string | number, frameDescription?: string): string {
  ...
  md += '\n';
  md += renderPromptBlock(promptObject?.image_prompt ?? promptObject?.ai_prompts?.image_prompt, 'Image Prompt', '      ');
  md += renderPromptBlock(promptObject?.video_prompt ?? promptObject?.ai_prompts?.video_prompt, 'Video Prompt', '      ');
  md += renderPromptBlock(promptObject?.audio_prompt ?? promptObject?.ai_prompts?.audio_prompt, 'Audio Prompt', '      ');
  md += renderPromptBlock(promptObject?.music_prompt ?? promptObject?.ai_prompts?.music_prompt, 'Music Prompt', '      ');
  md += renderPromptBlock(promptObject?.system_rules ?? promptObject?.ai_prompts?.system_rules, 'System Rules', '      ');
  return md + '\n';
}
            ...
            if (act.scenes && act.scenes.length > 0) {
              act.scenes.forEach((scene: any, sceneIdx: number) => {
                ...
                if (Array.isArray(scene.frames) && scene.frames.length > 0) {
                  ...
                  md += renderPromptBlock(scene.image_prompt ?? scene.ai_prompts?.image_prompt, 'Image Prompt');
                  md += renderPromptBlock(scene.video_prompt ?? scene.ai_prompts?.video_prompt, 'Video Prompt');
                  md += renderPromptBlock(scene.audio_prompt ?? scene.ai_prompts?.audio_prompt, 'Audio Prompt');
                  md += renderPromptBlock(scene.music_prompt ?? scene.ai_prompts?.music_prompt, 'Music Prompt');
                  md += renderPromptBlock(scene.system_rules ?? scene.ai_prompts?.system_rules, 'System Rules');
                }
